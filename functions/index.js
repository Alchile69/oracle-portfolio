...
// 🎯 NOUVELLE FONCTION COMMUNE - Calcul dynamique du régime
async function calculateDynamicRegime(country, config, API_KEYS) {
  let regime = config.regime_base;
  let confidence = config.confidence_base;
  let dataSource = 'config';

  console.log(`Calculating dynamic regime for country: ${country}`);
    ...
    try {
      ...
      if (pmiResponse.observations && pmiResponse.observations.length > 0) {
        ...
        if (pmiValue && pmiValue !== '.') {
          ...
          console.log(`PMI from FRED for ${country}:`, pmi);
      ...
      console.error(`PMI API Error for ${country}, using base regime:`, apiError.message);
...
// 📍 Market Stress Function - UNCHANGED
...
// 📍 Market Data Function - UNCHANGED
...
// 📍 Allocations Function - ENHANCED avec régime dynamique
exports.getAllocations = withCors(async (req, res, API_KEYS) => {
  try {
    ...
    // 🎯 UTILISE LA LOGIQUE DYNAMIQUE
    const dynamicRegime = await calculateDynamicRegime(country, config, API_KEYS);
    ...
    const response = {
      ...
      data: {
        regime: dynamicRegime.regime,
        confidence: dynamicRegime.confidence,
      ...
      data_source: dynamicRegime.dataSource,
      timestamp: new Date().toISOString(),
      last_update: new Date().toISOString()
    };
...
// 📍 Regime Function - UTILISE LA LOGIQUE COMMUNE
exports.getRegime = withCors(async (req, res, API_KEYS) => {
  try {
    ...
    // 🎯 UTILISE LA LOGIQUE DYNAMIQUE COMMUNE
    const dynamicRegime = await calculateDynamicRegime(country, config, API_KEYS);
...
// 📍 Countries Function - CORRIGÉE avec logique dynamique
exports.getCountries = withCors(async (req, res, API_KEYS) => {
  try {
    console.log("=== getCountries START ===");
    
    // 🎯 CALCUL DYNAMIQUE POUR CHAQUE PAYS
    const countries = [];
    
    for (const [code, config] of Object.entries(REGIME_CONFIG)) {
      const dynamicRegime = await calculateDynamicRegime(code, config, API_KEYS);
...
exports.getHealth = withCors(async (req, res, API_KEYS) => {
  try {
    ...
    const health = {
      ...
      regime_calculation: "dynamic_unified" // Nouvelle info
    };
